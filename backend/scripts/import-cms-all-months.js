...
/**
 * Get the date from a monthly zip filename
 * e.g., "nursing_homes_including_rehab_services_Jan2024.zip" -> "2024-01-01"
 */
function getMonthlyExtractDate(filename) {
  // Try MonthYear format (e.g., Jan2024, Feb2023)
  ...
  // Try MM_YYYY or YYYY_MM format
  const numericMatch = filename.match(/(\d{2})_(\d{4})|(\d{4})_(\d{2})/);
  if (numericMatch) {
    if (numericMatch[1] && numericMatch[2]) {
      return `${numericMatch[2]}-${numericMatch[1]}-01`;
    }
    if (numericMatch[3] && numericMatch[4]) {
      return `${numericMatch[3]}-${numericMatch[4]}-01`;
    }
  }
...
/**
 * Get extract date from yearly archive name (fallback)
 */
function getYearlyExtractDate(filename) {
...
/**
 * Extract and import ALL monthly zips from a yearly archive
 */
async function processYearlyArchive(zipPath) {
  const sourceName = path.basename(zipPath);
  console.log(`\n${'─'.repeat(60)}`);
  console.log(`Processing yearly archive: ${sourceName}`);
  ...
  // Find all nested monthly zips
  const nestedZips = fs.readdirSync(extractDir)
    .filter(f => f.endsWith('.zip'))
    .sort();

  if (nestedZips.length === 0) {
    // No nested zips - this IS the data directory
    console.log('  No nested zips - importing directly');
    const extractDate = getYearlyExtractDate(sourceName);
    if (extractDate) {
      await importMonthData(extractDir, extractDate, sourceName);
    }
    return 1;
  }
  ...
  for (const monthlyZip of nestedZips) {
    const monthlyZipPath = path.join(extractDir, monthlyZip);
    const extractDate = getMonthlyExtractDate(monthlyZip);

    if (!extractDate) {
      console.log(`  Skipping ${monthlyZip} - could not determine date`);
      continue;
    }
    ...
    if (existing.rows.length > 0) {
      console.log(`  Skipping ${monthlyZip} - already imported (${extractDate})`);
      continue;
    }

    console.log(`  Processing ${monthlyZip} (${extractDate})...`);

    // Extract monthly zip
    const monthDir = path.join(extractDir, `month_${extractDate}`);
    fs.mkdirSync(monthDir, { recursive: true });

    try {
      execSync(`unzip -q -o "${monthlyZipPath}" -d "${monthDir}"`, { stdio: 'pipe' });
      await importMonthData(monthDir, extractDate, monthlyZip);
      monthsImported++;
    } catch (err) {
      console.log(`    Error: ${err.message}`);
    }

    // Clean up month directory to save space
...
/**
 * Import data from a single month's directory
 */
async function importMonthData(dataDir, extractDate, sourceName) {
  const extractId = await getOrCreateExtract(extractDate, sourceName);

  // Find provider info file
  ...
  if (providerFiles.length > 0) {
    ...
    const imported = await importProviderInfo(providerFile, extractId);
    ...
    await pool.query(`
      UPDATE cms_extracts SET import_status = 'failed' WHERE extract_id = $1
    `, [extractId]);
  }
}

/**
 * Import current (non-archived) data
 */
async function importCurrentData(dataDir) {
  console.log(`\n${'─'.repeat(60)}`);
  console.log(`Processing current data: ${path.basename(dataDir)}`);

  // Parse date from directory name like "nursing_homes_including_rehab_services_12_2025"
  const match = dataDir.match(/(\d{2})_(\d{4})$/);
  let extractDate;
  if (match) {
    extractDate = `${match[2]}-${match[1]}-01`;
  } else {
    extractDate = new Date().toISOString().slice(0, 10);
  }

  await importMonthData(dataDir, extractDate, path.basename(dataDir));
}
...
async function detectEvents() {
  ...
  // Clear existing events for clean re-detection
  await pool.query('TRUNCATE TABLE facility_events');
...
async function main() {
  console.log('='.repeat(60));
  console.log('CMS HISTORICAL DATA IMPORT - ALL MONTHS');
  console.log('='.repeat(60));
  ...
  try {
    ...
    await pool.query('TRUNCATE TABLE facility_events');
    await pool.query('TRUNCATE TABLE quality_snapshots');
    await pool.query('TRUNCATE TABLE facility_snapshots CASCADE');
    await pool.query('TRUNCATE TABLE cms_extracts CASCADE');
    console.log('Existing data cleared');

    // Get historical archives sorted by year
    ...
    for (const archive of archives) {
      const monthsImported = await processYearlyArchive(archive);
      totalMonths += monthsImported;
    }

    // Import current data if available
    if (fs.existsSync(CURRENT_DATA_DIR)) {
      console.log('\nImporting current data...');
      await importCurrentData(CURRENT_DATA_DIR);
      totalMonths++;
    }

    // Detect events between all monthly snapshots
    await detectEvents();
    ...
    console.log('='.repeat(60));
    console.log(`\nMonthly extracts: ${summary.rows[0].extracts}`);
    console.log(`Total facility snapshots: ${summary.rows[0].snapshots}`);
