...
import { getFacilityComments, addFacilityComment, deleteFacilityComment } from '../../api/facilityService';
import MentionInput from '../common/MentionInput';
import { useAuth } from '../../context/UserContext';
import './FacilityCommentsSection.css';

const FacilityCommentsSection = ({ ccn, facilityName }) => {
  const { user } = useAuth();
  ...
  const loadComments = async () => {
    try {
      setLoading(true);
      const response = await getFacilityComments(ccn);
      if (response.success) {
  ...
  const handleSubmitComment = async (e) => {
    ...
    setSubmitting(true);
    try {
      const response = await addFacilityComment(ccn, {
        comment: newComment,
  ...
  const handleDeleteComment = async (commentId, parentId = null) => {
    ...
    try {
      const response = await deleteFacilityComment(ccn, commentId);
      if (response.success) {
        if (parentId) {
  ...
  const renderComment = (comment, isReply = false, parentId = null) => {
    ...
    return (
      <div key={comment.id} className={`facility-comment ${isReply ? 'reply' : ''}`}>
  ...
  return (
    <div className="facility-comments-section">
      ...
      <form onSubmit={handleSubmitComment} className="add-comment-form">
        <MentionInput
          ...
          placeholder="Add a comment about this facility... Use @ to mention someone"
      ...
      <div className="comments-list">
        {loading ? (
          ...
          <div className="comments-empty">
            ...
            <p>No comments yet</p>
            <span>Be the first to comment on this facility</span>
          </div>
        ) : (
...
export default FacilityCommentsSection;
